...
fn = "C:/Users/etudiant/Desktop/ENSE3/Stage 2A/SIG/Tram Khnar ID Map - 009/Layers/Households/Household IDs.shp"
...
if check_digits_all == 0:
    print("\nMissing household IDs", sorted(missing_ids, key = len))
...
if check_digits_all == 0:
    print("Duplicate household IDs", sorted(duplicated_ids, key = len))
    if len(duplicated_ids) == 0:
        show_popup_message("Information", "No duplicate HH IDs on the map.", "information")
    elif len(duplicated_ids) == 1:
        ...
        print("Duplicate households not fixed")
        show_popup_message("Error", "There are several duplicate HH IDs on the map\nThis program can solve only one duplicate case.", "error")
